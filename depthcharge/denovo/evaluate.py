...
def best_aa_match(orig_seq, pred_seq, aa_dict):
    ...
    while cnt_pred < len(pred_seq) and cnt_orig < len(orig_seq):
        ...
        if abs(spectrum_utils.utils.mass_diff(pred_prefix_mass_list[cnt_pred],orig_prefix_mass_list[cnt_orig], mode_is_da = True)) < cum_aa_threshold or abs(spectrum_utils.utils.mass_diff(pred_suffix_mass_list[cnt_pred],orig_suffix_mass_list[cnt_orig], mode_is_da = True)) < cum_aa_threshold:

            if abs(spectrum_utils.utils.mass_diff(pred_aa_mass, orig_aa_mass, mode_is_da = True)) < single_aa_threshold:
                aa_match += [1]
            else:
                aa_match += [0]
        ...
        elif spectrum_utils.utils.mass_diff(pred_prefix_mass_list[cnt_pred],orig_prefix_mass_list[cnt_orig], mode_is_da = True) > 0:
            cnt_orig += 1
...
def find_aa_match_single_pep(orig_seq, pred_seq, aa_dict):
    ...
    while cnt_pred < len(pred_seq) and cnt_orig < len(orig_seq):
        ...
        if abs(spectrum_utils.utils.mass_diff(pred_cum_aa_mass,orig_cum_aa_mass, mode_is_da = True)) < cum_aa_threshold:
        ...
        elif spectrum_utils.utils.mass_diff(pred_cum_aa_mass,orig_cum_aa_mass, mode_is_da = True) > 0:
            cnt_orig += 1
            orig_cum_aa_mass += orig_aa_mass
